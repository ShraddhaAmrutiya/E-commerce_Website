...
const addReview = async (req, res) => {
  try {
    ...
    let existingReview = await reviewModel_1.default.findOne({ product: productId, user: userId });
    ...
    if (existingReview) {
      ...
      // Create a new review
      existingReview = new reviewModel_1.default({
        product: productId,
        user: userId,
        rating,
        comment,
    ...
    const reviews = await reviewModel_1.default.find({ product: productId });
    const averageRating = reviews.reduce((sum, review) => sum + review.rating, 0) / reviews.length;
    product.rating = parseFloat(averageRating.toFixed(1));
    await product.save();

    res.status(existingReview.isNew ? 201 : 200).json({
      message: req.t(existingReview.isNew ? "review.reviewSuccess" : "review.reviewUpdated"),
